...
自动在 Claude Code 会话间循环：执行 → 保存 WIP → 恢复 → 继续，直到目标达成或触发熔断。通过 `.claude-flow/wip.md` 文件在会话间传递进度。

> **何时用 `/deep-task` vs `persistent-solve.py`**：大多数 L 级任务，`/deep-task` 在单会话内就能完成（并行 Agent + 模型路由）。只有真正超出单会话预算的 XL 级任务才需要 `persistent-solve.py` 做外层循环。
