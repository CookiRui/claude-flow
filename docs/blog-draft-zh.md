...
Phase 2.5: 预算门控 — 从 DAG 估算费用，>$3 需用户确认
Phase 3: 并行执行 — 无冲突的任务同时跑
Phase 4: 三级验证
         L1: 每个子任务自检
         L2: Reviewer ↔ Executor 对抗循环（收敛检测：blocker 数必须递减，否则停止并升级给用户）
         L3: 端到端集成验证
Phase 5: 元学习 — 记录到 .claude-flow/learnings/{domain}.md（带相关性评分 + 自动清理）
```

Phase 0 很关键：不是每个任务都要走完 8 层——S 级任务（单文件、可自动验证）直接执行+提交，跳过整个引擎。Phase 5 把经验写入项目的 `.claude-flow/learnings/{domain}.md`（按领域分文件，带相关性评分和自动清理），系统用得越多越聪明。
...
Reviewer 通过（或 blocker 数不再递减时升级给用户）
```
