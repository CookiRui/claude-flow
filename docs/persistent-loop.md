# 跨会话持久化循环 + WIP 机制

单次 Claude Code 会话有预算限制（token/时间/上下文窗口）。复杂任务做到一半预算耗尽怎么办？**不是停下来等人——而是自动恢复继续推进。**

---

## 与 `/deep-task` 的关系

| 方案 | 范围 | 适用场景 |
|------|------|----------|
| `/deep-task` | **单会话内**：DAG 分解 → 并行 Agent → 三级验证 → 元学习 | 大多数 L 级任务，单会话可完成 |
| `persistent-solve.py` | **跨会话**：外层循环，会话间通过 WIP 文件传递进度 | XL 级任务，超出单会话预算 |

它们是互补的：`/deep-task` 是会话内的执行引擎，`persistent-solve.py` 是会话间的调度器。大多数情况下 `/deep-task` 足够，只有真正超大的任务才需要外层循环。

---

## 核心问题：单会话的 3 个硬退出点

| 退出点            | 原因                    | 后果             |
|-------------------|-------------------------|------------------|
| `attempts >= MAX` | 单任务尝试 2-3 次后放弃 | 任务未完成就停止 |
| `budget` 耗尽     | token/时间/步数用完     | 整体目标未达成   |
| `escalate(L3)`    | 信心太低，交接给人类    | 等人来才能继续   |

更关键的架构限制：**Claude Code 是单会话模型**。会话结束后没有外部进程自动重启下一轮。

---

## 解决方案：两层循环架构

```
┌─────────────────────────────────────────────────────────┐
│  外层：持久化循环（跨会话）                               │
│  由外部调度器驱动，负责"不达目的不停止"                     │
│                                                         │
│  ┌───────────────────────────────────────────────────┐  │
│  │  内层：单会话执行（autonomous_solve）               │  │
│  │  在预算和上下文范围内尽力推进                        │  │
│  │  遇到硬约束 → 保存 WIP → 退出                      │  │
│  └───────────────────────────────────────────────────┘  │
│                                                         │
│  外层检查：目标达成了吗？                                 │
│  ├─ 是 → 完成，退出循环                                  │
│  └─ 否 → 从 WIP 恢复，启动新会话，继续推进               │
└─────────────────────────────────────────────────────────┘
```

---

## 三种运行模式

| 模式               | 怎么用                                        | 适合场景                         |
|--------------------|-----------------------------------------------|----------------------------------|
| **单会话增强**     | `/deep-task "目标"`                           | L 级任务，并行 Agent 单会话完成  |
| **全自动跨会话**   | `python scripts/persistent-solve.py "目标"`   | XL 级任务，超出单会话预算        |
| **半自动**         | 每轮结束后人工恢复 WIP                        | 需要中途审查的任务               |

---

## 全自动模式：使用方式

```bash
# Run the persistent loop scheduler
python scripts/persistent-solve.py "Stabilize game frame rate at 60fps"

# The script will automatically:
#   Round 1: Launch Claude Code → execute until budget runs out → save WIP
#   Round 2: Resume from WIP → continue executing → save WIP
#   Round 3: Resume from WIP → goal achieved → exit
#
# Max 10 rounds, or stops when circuit breaker triggers

# Options
python scripts/persistent-solve.py "goal" --max-rounds 5
python scripts/persistent-solve.py "goal" --max-time 1800  # 30 min total
```

---

## 内层自救机制

当某个子任务多次失败时，内层会尝试 3 级自救：

```
失败 → 自救 1: 换一个完全不同的角度
  ↓ 仍失败
      → 自救 2: 把当前子任务拆得更细
  ↓ 仍失败
          → 自救 3: 深度搜索学习后重试
  ↓ 仍失败
              → 保存 WIP，标记 [NEED_HUMAN]
```

只有所有自救手段都用尽后，才会标记需要人类介入。

---

## 熔断保护

全自动模式有安全限制，防止无限烧钱：

| 熔断条件         | 阈值   | 触发后             |
|------------------|--------|--------------------|
| 最大轮次         | 10     | 停止循环           |
| 最大总时间       | 1 小时 | 停止循环           |
| 最大费用         | $5     | 停止循环           |
| 连续无进展       | 3 轮   | 停止，提示人工接手 |
| 同一错误重复出现 | 3 次   | 停止，提示换方向   |

---

## WIP 机制

WIP（Work In Progress）是持久化循环的状态存储格式。**固定路径：`.claude-flow/wip.md`**。

脚本和 Claude 之间通过这个文件"握手"：
- 脚本在 prompt 中告诉 Claude WIP 文件路径
- Claude 在每轮结束前写入进度
- 脚本读取 WIP 判断状态，并注入下一轮 prompt

### 保存时机

- 预算即将耗尽时
- 升级给人类时（Level 3）
- 用户主动要求保存
- 目标达成时（status 改为 done）

### WIP 文件格式

```yaml
---
status: active              # active | need_human | done
goal: "目标描述"
round: 3                    # 当前第几轮
---

## Completed
- [x] 定位瓶颈（DrawCall 40%, GC 25%, Physics 15%）
- [x] 优化 DrawCall（800 → 320）

## Remaining
- [ ] 优化 GC（依赖: 无）
- [ ] 优化 Physics（依赖: 无）
- [ ] 最终验证（依赖: GC, Physics）

## Strategies Tried
- DrawCall: SRP Batcher 失败（Built-in RP）→ 残值: 确认渲染管线类型
- DrawCall: Static/Dynamic Batching 成功

## Constraints
- 不能切换渲染管线
- 不能降低画质

## Next Steps
1. 优先处理 GC（占比 25%，预期收益最大）
2. 搜索 GC 优化最佳实践
```

### 状态判断优先级

脚本按以下优先级判断每轮结果：

1. **WIP status 字段** — `done` / `need_human` / `active`（主要依据）
2. **WIP [x] 计数变化** — 判断是否有实际进展
3. **WIP 内容变化** — 兜底检测
4. **stdout 中的标记** — `[GOAL_ACHIEVED]` 作为最后 fallback

### WIP 恢复流程

1. 脚本读取 `.claude-flow/wip.md`
2. 将 WIP 全文注入下一轮 prompt
3. Claude 从 "Next Steps" 开始执行，不重复已完成的工作
4. Claude 更新 WIP 文件（轮次 +1，追加新完成的任务）

---

## 实际落地方式

| 方案                         | 实现                                | 自动化程度 |
|------------------------------|-------------------------------------|------------|
| **方案 A: 脚本调度**         | `python scripts/persistent-solve.py` | 全自动     |
| **方案 B: GitHub Actions**   | Push WIP 后触发 Action              | 全自动     |
| **方案 C: 手动恢复**         | 人工在 Claude Code 中 `Resume WIP`  | 手动       |

**推荐**：日常使用方案 C（手动），重要任务用方案 A（脚本调度）。
