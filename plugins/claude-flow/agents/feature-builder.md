...
1. Read `.claude/constitution.md` if it exists — understand all architectural constraints.
...
3. Run `repo-map.py --incremental` if available to update layered code map.
4. Read `.repo-map/L0.md` for project overview (if it exists).
5. Identify which modules are affected by this feature.
6. Read the files most likely to be affected. Do not modify anything yet.
7. Output a one-paragraph understanding summary. If confidence < 0.8, ask clarifying questions before continuing.
...
- [ ] No lint errors.
- [ ] Clean build, no warnings treated as errors.
...
Create a PR from the worktree branch into the base branch (typically `main` or `develop`).
...
Checked against all articles — no violations found.
...
- Do not skip the worktree setup — never commit feature work directly to the base branch.
