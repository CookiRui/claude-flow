...
1. Read `.claude/constitution.md` if it exists — understand architectural constraints (some affect what is testable in isolation).
...
3. Identify the test framework and conventions for this project:
   - Detect from existing test files, manifest files (package.json scripts, pytest.ini, etc.)
   - Identify test directory and naming conventions from existing tests
...
(Only if the component is on a performance-critical path.)
...
2. Run the tests scoped to the new tests.
