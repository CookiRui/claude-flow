...
1. Read `.claude/constitution.md` if it exists — the constitution defines the project's non-negotiable constraints. Every critical finding must cite the relevant article.
2. Read `.claude/rules/` if it exists — understand project-specific coding style rules.
...
Check which **Code Tier** the file belongs to (see `REVIEW.md` → Code Tiers if available):

- **Production tier**: Apply performance rules at full severity (BLOCKER/WARNING)
...
Re-read each article of `.claude/constitution.md` (if it exists). For every article, check:
