...
4. **Build context** (three layers: constraints → codebase → integration):
   ...
   **Layer A — Constraints:**
   - Read constitution (`.claude/constitution.md`) if it exists
   - Read all rules in `.claude/rules/*.md` if they exist
   - Load relevant Skills

   **Layer B — Codebase understanding:**
   - Run `repo-map.py --format md --no-refs` if available (generates code map)
...
1. **Consumer coverage**: For each file being created/modified in the DAG,
   ...
   - Do any hooks need to call the new scripts?
   - Do any agents or commands need updating?

2. **Config files**: If the DAG creates new generated directories or output files,
   verify DAG includes .gitignore/.claudeignore updates.

3. **Documentation**: If the DAG changes public-facing CLI or functionality,
   verify DAG includes README.md updates.

4. **Pattern check**: Read existing scripts that serve similar purposes.
...
4. Constitution: read .claude/constitution.md if it exists, check compliance article by article
...
Rules:
- Read .claude/constitution.md before making changes (if it exists)
...
### 5.2 Write to Claude memory

If the execution revealed a cross-project insight, save it to Claude's memory system.
